...
class DatasetCard(QFrame):
    ...
    def create_ui(self):
        ...
        status_label = QLabel("✓ Téléchargé" if self.dataset.is_downloaded else "⬇ Disponible")
        status_label.setStyleSheet(
            "color: green; font-weight: bold;" if self.dataset.is_downloaded 
        ...
        if self.dataset.is_downloaded:
            ...
            """)
            delete_btn.clicked.connect(lambda: self.delete_requested.emit(self.dataset.id))
            ...
            """)
            download_btn.clicked.connect(lambda: self.download_requested.emit(self.dataset.id))
...
• Total: {stats['total_size_mb']:.1f} MB"""
    ...
    def start_download(self, dataset_id: str):
        ...
        try:
            ...
            thread.progress_updated.connect(lambda info: self.update_progress(dataset_id, info))
    ...
    def download_finished(self, dataset_id: str, success: bool):
        ...
        # Message de résultat
        if success:
            QMessageBox.information(self, "Téléchargement", f"Dataset {dataset_id} téléchargé avec succès!")
        else:
            QMessageBox.critical(self, "Erreur", f"Échec du téléchargement de {dataset_id}")
    ...
    def delete_dataset(self, dataset_id: str):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                if success:
                    QMessageBox.information(self, "Suppression", "Dataset supprimé avec succès!")
                    self.refresh_datasets()
                else:
    ...
    def create_personal_dataset(self):
        ...
        try:
            ...
            description, ok = QInputDialog.getText(self, "Nouveau Dataset", "Description:")
            if not ok:
                description = ""
            ...
            if dataset_id:
                QMessageBox.information(self, "Dataset Créé", f"Dataset '{name}' créé avec succès!")
                self.refresh_datasets()
            else:
                QMessageBox.critical(self, "Erreur", "Erreur lors de la création du dataset")
