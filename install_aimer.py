...
# Versions épinglées
NUMPY_VER      = "1.26.4"
OPENCV_WHL_URL = ("https://files.pythonhosted.org/packages/2f/ae/"
                  "b861a2f8093d34c4714bc889eb0bb65fd0fc71e782a392e81ee4d42a6fdd/"
                  "opencv_python-4.8.1.78-cp310-cp310-win_amd64.whl")
TORCH_PKGS     = ("torch==2.0.1+cpu "
                  "torchvision==0.15.2+cpu "
                  "torchaudio==2.0.2+cpu "
                  "--index-url https://download.pytorch.org/whl/cpu")
DETECTRON_WHL  = ("https://cdn.jsdelivr.net/gh/myhloli/wheels@main/assets/whl/"
...
def main() -> int:
    ...
    run(f"{sys.executable} -m pip install numpy=={NUMPY_VER}", "Installation NumPy 1.x compatible")

    # 3. PyTorch CPU + libs
    run(f"{sys.executable} -m pip install {TORCH_PKGS}", "Installation PyTorch CPU 2.0.1")

    # 4. OpenCV 4.8.1 wheel (sans dépendances pour ne pas ré-installer NumPy 2)
    run(f"{sys.executable} -m pip install \"{OPENCV_WHL_URL}\" --no-deps",
    ...
    run(f"{sys.executable} -m pip install \"{DETECTRON_WHL}\"", "Installation Detectron2 0.6 (wheel)")
