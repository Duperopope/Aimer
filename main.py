    ...
    try:
        ...
        if args.check:
            ...
            if "Detectron2" in missing:
                ...
                print("Installation: pip install 'git+https://github.com/facebookresearch/detectron2.git'")
                return False
            ...
            if result.instances:
                ...
                print(f"Temps: {result.performance_metrics.get('inference_time_ms', 0):.1f}ms")
...
Exemples d'utilisation:
  ...
  python main.py --cli --detect image.jpg --task instance_segmentation
    ...
    parser.add_argument("--check", action="store_true",
                       help="Vérification système")
    parser.add_argument("--detect", type=str, metavar="IMAGE",
                       help="Détection sur image")
    
    # Arguments de configuration
    parser.add_argument("--task", type=str, default="detection",
                       choices=["detection", "instance_segmentation", 
                               "panoptic_segmentation", "keypoint_detection"],
                       help="Type de tâche")
